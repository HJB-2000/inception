...
This document describes the internal setup, service responsibilities, and operational details of the Inception project as it is currently configured.

## Scope

This repository is meant to be a self-contained Docker Compose deployment for a WordPress-based web application. The current active build includes the following services:
...
Grafana and Prometheus are not part of the current build and have been removed from the Compose stack.

## Compose architecture

The project uses a single Compose file at `srcs/docker-compose.yml` to orchestrate all services. Services are connected through the `inception` bridge network.
...
All application services belong to the same Docker network so they can resolve each other by container name when needed.

### Volumes

Persistent data is kept in bind-mounted volumes pointing to host directories under `/home/jbahmida/data/`.

- `wordpress_data` -> `/home/jbahmida/data/wordpress`
- `mariadb_data` -> `/home/jbahmida/data/mariadb`
- `uptime-kuma_data` -> `/home/jbahmida/data/uptime_kuma`

These are defined in the Compose file and persist state across container restarts.

## Service breakdown

### MariaDB

- Image: `mariadb:jbahmida`
- Container: `mariadb`
- Purpose: stores the WordPress database and related data.
- Healthcheck: checks MySQL availability with `mysqladmin ping`.
- Secrets: database password and root password.

### WordPress

- Image: `wordpress:jbahmida`
- Container: `wordpress`
- Purpose: runs the PHP-FPM application for WordPress.
- Depends on: MariaDB and Redis health checks.
- Volumes: WordPress files persisted at `/var/www/html`.
- Secrets: database password and WordPress credentials.

### NGINX

- Image: `nginx:jbahmida`
- Container: `nginx`
- Purpose: TLS termination and reverse proxy for WordPress.
- Exposes port `443` to the host.
- Depends on healthy WordPress service.

### Redis

- Image: `redis:jbahmida`
- Container: `redis`
- Purpose: cache layer for app acceleration and WordPress integration.
...
- Image: `ftp:jbahmida`
- Container: `ftp`
- Purpose: file transfer access to the WordPress document root.
- Ports: `21` and passive range `21100-21110`.
- Secret: FTP password.

### Static site

- Image: `static-site:jbahmida`
- Container: `static-site`
- Purpose: simple static HTML page served by the Docker environment.
- Port: `8081`.

### Adminer

- Image: `adminer:jbahmida`
- Container: `adminer`
...
- Image: `cadvisor:jbahmida`
- Container: `cadvisor`
- Purpose: container metrics collection.
- Port: `8080`.
- Runs with privileged access to inspect Docker host metrics.

### Uptime Kuma

- Image: `uptime-kuma:jbahmida`
- Container: `uptime-kuma`
- Purpose: uptime and service monitoring.
- Port: `3001`.
- Uses a bind-mounted data directory for its application database.

## Security and secret management

The project uses Docker secrets defined in the Compose file. These point to files in the `secrets/` directory, such as:
...
This prevents secrets from being hard-coded directly into Dockerfiles or Compose definitions.

## Build workflow

The Makefile contains the top-level workflow used to build and manage the environment:
...
`make all` creates the required host directories and starts the stack with Docker Compose. `make re` performs a full reset and rebuild.

## Operational notes

- Compose service ordering uses health conditions to avoid race conditions.
- Database and WordPress data are persistent across container rebuilds.
- TLS is configured at the NGINX layer and is expected to serve the HTTPS site.
- The stack is intentionally structured around a small set of supporting services, with the removed monitoring stack not included in the active build.

## Developer guidelines

When adding or removing a service:

1. Update the Compose service list in `srcs/docker-compose.yml`.
2. Keep the `inception` network consistent.
3. Ensure any new volume path is created on the host and matches the intended data directory.
4. Update documentation so README and DEV_DOC reflect the exact live configuration.
5. Keep secrets and environment values separate from source files.

## Current status

The active project configuration is stable and matches the documented service list in this file. Grafana and Prometheus have been deliberately removed from the running build, and the repository documentation has been aligned to that current state.
